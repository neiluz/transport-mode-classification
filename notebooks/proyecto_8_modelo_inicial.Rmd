...
### ANOVA tras el análisis de colinealidad (VIF)
...
winsorize <- function(x) {
  q1 <- quantile(x, 0.05)  # Percentil 1
  q99 <- quantile(x, 0.95)  # Percentil 99
  pmax(pmin(x, q99), q1)  # Limitar valores fuera de estos percentiles
}

# Lista de columnas a winsorizar
columns_to_winsorize <- c("gyroZ.rad.s.", "Roll.rads.", "m13", "gyroX.rad.s.", "gyroY.rad.s.",  "accelX.g.", "m32", "qX", "magZ.µT.", "qY", "accelUserX.g.", 
...
Como se puede observar, la **winsorización no produjo cambios significativos en los valores extremos**, lo que indica que los outliers siguen presentes en los datos. Dado que estos valores pueden afectar el rendimiento del modelo y la estabilidad del entrenamiento, se opta por una estrategia de **imputación basada en la media**. Este método permite suavizar el impacto de los valores atípicos sin eliminar información clave, asegurando que las variables con distribuciones sesgadas no introduzcan ruido excesivo en la clasificación del tipo de transporte.

```{r}
# Lista de columnas a winsorizar (imputar outliers con la mediana)
columns_to_winsorize <- c("gyroZ.rad.s.", "Roll.rads.", "m13", "gyroX.rad.s.", "gyroY.rad.s.",  
                          "accelX.g.", "m32", "qX", "magZ.µT.", "qY", "accelUserX.g.", 
                          "accelUserZ.g.", "accelZ.g.", "magY.µT.", "m33", "accelUserY.g.")


# Filtrar solo las columnas que realmente existen en `data`
columns_to_winsorize <- columns_to_winsorize[columns_to_winsorize %in% names(data)]
print(columns_to_winsorize)  # Verifica qué columnas quedan después del filtro
...
# Aplicar la imputación solo a las variables que existen en `data`
data_imputed <- data %>%
  mutate(across(all_of(columns_to_winsorize), replace_outliers))

# Verificar que los valores extremos fueron reemplazados
...
### **1️⃣ Dinámicas de Movimiento (Aceleración y Giroscopios)**

Estas variables son esenciales para **describir patrones característicos de movimiento**, como **aceleración, frenado y giros**, diferenciando entre tipos de transporte como automóviles, bicicletas o aviones.

-   ✅ **Aceleración:** `accelX.g.`, `accelZ.g., accelUserX.g.`, `accelUserY.g.`, `accelUserZ.g.`

-   ✅ **Giroscopios (Rotaciones):** `Roll.rads.`, `Pitch.rads.`, `Yaw.rads.`

📌 **Motivo de la selección:**\
Las aceleraciones y rotaciones son **fundamentales** para identificar la dinámica del transporte, ya que diferentes medios de transporte presentan patrones específicos en estas variables.

### **2️⃣ Contexto Ambiental**

Estas variables aportan información **sobre las condiciones del entorno**, diferenciando **medios terrestres y aéreos**.

-   ✅ **Presión atmosférica:** `Pressure.kilopascals.`

-   ✅ **Altitud relativa:** `RelativeAltitude.meters.`

📌 **Motivo de la selección:**\
La presión y altitud **permiten diferenciar transporte terrestre y aéreo**, proporcionando información crítica sobre la ubicación y altitud del movimiento.

### **3️⃣ Componentes de Movimiento**

Estas variables describen **transformaciones dinámicas en la trayectoria**, complementando la información de aceleración y rotación.

-   ✅ `m12`, `m13`, `m22`, `m33`

📌 **Motivo de la selección:**\
Aunque no están entre las más importantes individualmente, **mejoran la precisión en la identificación del tipo de transporte**, especialmente en escenarios más complejos.

## **📌 Variables Excluidas**

Se decidió excluir variables con **baja relevancia relativa** o **redundancia**:

-   ❌ **Aceleración del usuario:** `accelUserY.g.` (redundantes con `accelX.g.` y `accelZ.g.`).

-   ❌ **Giroscopios con menor relevancia:** `gyroX.rad.s.`, `gyroZ.rad.s.` (menor importancia en `MeanDecreaseGini`).

-   ❌ **Variables de orientación `q` eliminadas:** `qX`, `qY`, `qZ`, `qW` (baja importancia en RF y redundancia en orientación).
...
![](images/clipboard-2531371232.png)
