---
title: "Análisis Exploratorio de Datos (EDA) - Proyecto de Transporte -* Dataset  Avion - Despegue"
...
### **2. Reemplazar valores `unknown` en `ActivityType`**
...
# 1. Comparar `gyroZ.rad.s.`
boxplot(data_analysis$gyroZ.rad.s., main = "Distribución General\n(gyroZ.rad.s.)", col = "lightblue", cex.main = cex_title)
boxplot(outlier_data$gyroZ.rad.s., main = "Outliers en\n(gyroZ.rad.s.)", col = "salmon", cex.main = cex_title)

# 2. Comparar `gyroY.rad.s.`
boxplot(data_analysis$gyroY.rad.s., main = "Distribución General\n(gyroY.rad.s.)", col = "lightblue", cex.main = cex_title)
boxplot(outlier_data$gyroY.rad.s., main = "Outliers en\n(gyroY.rad.s.)", col = "salmon", cex.main = cex_title)

# 3. Comparar `accelUserX.g.`
boxplot(data_analysis$accelUserX.g., main = "Distribución General\n(accelUserX.g.)", col = "lightblue", cex.main = cex_title)
boxplot(outlier_data$accelUserX.g., main = "Outliers en\n(accelUserX.g.)", col = "salmon", cex.main = cex_title)

# 4. Comparar `accelUserY.g.`
boxplot(data_analysis$accelUserY.g., main = "Distribución General\n(accelUserY.g.)", col = "lightblue", cex.main = cex_title)
boxplot(outlier_data$accelUserY.g., main = "Outliers en\n(accelUserY.g.)", col = "salmon", cex.main = cex_title)

# 5. Comparar `magZ.µT.`
boxplot(data_analysis$magZ.µT., main = "Distribución General\n(magZ.µT.)", col = "lightblue", cex.main = cex_title)
boxplot(outlier_data$magZ.µT., main = "Outliers en\n(magZ.µT.)", col = "salmon", cex.main = cex_title)
...
**Extensión del Análisis: Relaciones Dinámicas entre las Fases del Vuelo**

Dado que el análisis de colinealidad confirmó la existencia de dependencias entre ciertas variables clave, se decide ampliar el estudio hacia la evaluación de **relaciones dinámicas** entre las distintas fases del vuelo. Aunque los datos disponibles **corresponden exclusivamente a la fase de despegue**, el análisis de estas relaciones permitirá comprender mejor cómo evolucionan las variables dinámicas durante este proceso crítico.

El objetivo de este análisis es:

-   **Evaluar la interacción entre aceleración, velocidad angular y altitud** en el contexto del despegue.

-   **Identificar patrones en los cambios de inclinación (`Pitch`), balanceo (`Roll`) y guiñada (`Yaw`)** a medida que el avión gana altitud.

-   **Explorar la influencia de los outliers previamente detectados**, determinando si representan eventos significativos en la dinámica del despegue.

Esta exploración contribuirá a una mejor interpretación de los datos y facilitará la toma de decisiones sobre cómo tratar la información en modelos futuros, asegurando que los patrones dinámicos sean correctamente capturados y utilizados.
...
data_copy <- data
# Crear etiquetas de fases del vuelo en la copia
data_copy <- data_copy %>%
...
ggplot(data_copy, aes(x = FlightPhase, y = accelX.g., fill = FlightPhase)) +
  geom_boxplot() +
...
ggplot(data_copy, aes(x = FlightPhase, y = Pitch.rads., fill = FlightPhase)) +
  geom_boxplot() +
...
ggplot(data_copy, aes(x = RelativeAltitude.meters., y = Pitch.rads., color = FlightPhase)) +
  geom_point() +
  labs(
...
ggplot(data_copy, aes(x = accelX.g., y = Roll.rads., color = FlightPhase)) +
  geom_point() +
  labs(
...
correlations_by_phase <- data_copy %>%
...
1.  **Eliminación de Columnas y Transformaciones Básicas**

    -   Se eliminaron algunas variables irrelevantes para la fase de despegue.

    -   Se realizaron transformaciones de tipo de dato (`chr` a `date` en variables temporales).

    -   Se renombraron algunos registros para mejorar la legibilidad del dataset.

2.  **Análisis Exploratorio de la Naturaleza de los Datos**

    -   Se identificaron **outliers en variables clave** (`gyroZ.rad.s.`, `gyroY.rad.s.`, `accelUserX.g.`, `accelUserY.g.`, `magZ.µT.`), pero no se han imputado ni eliminado.

    -   Se analizaron **correlaciones entre variables**, detectando posibles casos de colinealidad.

    -   Se planteó la posibilidad de estudiar las **relaciones dinámicas en el despegue**, aunque no se han definido fases de vuelo dentro del dataset.
...
```{r}
write.csv(data, "dataset_resultante_despegue_avion.csv", row.names = FALSE)
